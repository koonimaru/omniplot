...
def radialtree(
    ...
    **kwargs
    ...
    Z = sch.dendrogram(Y, labels=_labels, color_threshold=t, ax=_ax)
...
def catplot(
    ...
    **kwargs
    ...
    if kind == "violin":
        sns.violinplot(
            data=df, x=x, y=y, order=xorder, inner=inner, ax=ax, color=color, **violinkw
        )
    elif kind == "box":
    ...
    for i, pval in enumerate(pvals):
        ...
        if significance == "numeric":
            ...
            if annotate == "NA":
                annotate = keys[-1]
        plt.text(
            (pval[1] + pval[2]) / 2, ymax + (ymax - ymin) * (0.06 + i * 0.051), annotate
        )
    if significance == "symbol":
        ax.annotate(
            "\n".join(
                [
                    "{}: p < {:.2E}".format(k, 10 ** (-significance_ranges[k]))
                    for k in keys
                ]
            ),
            xy=(0.9, 0.9),
    ...
    if len(pairs) != 0:
        for p1, p2 in pairs:
            ...
            if test == "wilcoxon" or test == "ttest_rel":
                ...
                _, pval, _, _ = statstest(
                    p1val, p2val, alternative=alternative, **kwargs
                )
            elif test == "ttest_ind":
                _, pval = statstest(
                    p1val, p2val, alternative=alternative, equal_var=equal_var, **kwargs
                )
    ...
    if len(pairs) != 0:
        for i, pval in enumerate(pvals):
            if orientation == "vertical":
                ax.plot(
                    [pval[1], pval[2]],
                    [ymax * (1.05 + i * 0.05), ymax * (1.05 + i * 0.05)],
                    color="black",
                )
...
def _violinplot(
    ...
    for i, X in enumerate(mat):
        ...
        if orientation == "horizontal":
            ...
            ax.fill(
                [i, i - 0.05, i - 0.05, i + 0.05, i + 0.05, i],
                [q1, q1, q3, q3, q1, q1],
                lw=10,
                color=box_color,
            )
...
def lineplot(
    ...
    if split == False:
        ...
        for _y in y:
            ax.plot(
                X, df[_y], color=lut[_y], label=_y, linestyle=line_lut[_y], **plotkw
            )
            if estimate == "moving_average":
                ax.plot(
                    X,
                    df[_y].rolling(window=int(X.shape[0] / 10), center=True).mean(),
                    color="black",
                    linestyle="dashed",
                )
                ...
                ax.plot(
                    X,
                    df[_y].rolling(window=int(X.shape[0] / 10), center=True).median(),
                    color="black",
                    linestyle="dashed",
                )
                ...
                (
                    ...
                    std_error,
                    MSE,
                ) = _robust_regression(X, df[_y].fillna(0), plotline_X, robust_param)
                _stats[_y] = {
                    "model": fitted_model,
                    "summary": summary,
                    "coef": coef,
        ...
        if isinstance(ax, type(None)):
            fig, ax = plt.subplots(
                nrows=rows_cols[0], ncols=rows_cols[1], figsize=figsize
            )
            ax = ax.flatten()
...
def _set_axis(
    ax, x, xlabel, ylabel, xunit, yunit, xformat, yformat, logscalex, logscaley, title
):
    if xlabel != "":
